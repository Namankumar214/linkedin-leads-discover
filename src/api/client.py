...
class LeadsAPIClient:
    ...
    async def get_profile_urn(self, api: AsyncLinkdAPI, username: str) -> Tuple[bool, Optional[str], Optional[str]]:
        """
        Get profile URN from username
        Returns: (success, urn, error_message)
        """
        ...
        urn = response['data'].get('urn')
        if not urn:
            return False, None, "URN not found in profile data"
