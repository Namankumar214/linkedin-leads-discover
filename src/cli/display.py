...
def show_profiles_table(profiles: List[Dict], max_rows: int = 20):
    ...
    table.add_column("Headline", style="white", width=40)
    table.add_column("URN", style="dim", width=25)
    ...
    for i, profile in enumerate(profiles[:max_rows]):
        ...
        headline = profile.get('headline', 'N/A')[:37] + "..." if len(profile.get('headline', '')) > 40 else profile.get('headline', 'N/A')
        urn = profile.get('urn', 'N/A')[:22] + "..." if len(profile.get('urn', '')) > 25 else profile.get('urn', 'N/A')

        table.add_row(depth, name, headline, urn)
...
def show_profiles_tree(profiles: List[Dict], max_children_per_node: int = 5):
    ...
    def get_profile_label(profile: Dict) -> str:
        ...
        if not name:
            name = profile.get('publicIdentifier', 'Unknown')
        ...
        if children_count > 0:
            return f"[green]{name}[/] [dim]│[/] [yellow]{headline}[/] [dim]({children_count} discovered)[/]"
        else:
            return f"[green]{name}[/] [dim]│[/] [yellow]{headline}[/]"
